...
use ark_relations::r1cs::{ConstraintMatrices, ConstraintSynthesizer, ConstraintSystemRef};
...
#[derive(Clone)]
pub struct R1CS<C: ConstraintSynthesizer<Fq>> {
...
impl<C: ConstraintSynthesizer<Fq>> R1CS<C> {
...
impl<C: ConstraintSynthesizer<Fq>> Arithmetization for R1CS<C> {
    ...
    fn synthesize(&mut self, params: Fr, pc: usize, i: usize, cs: &mut Self::ConstraintSystem) {
        ...
        let (comm_W2, hash) = self.latest.alloc_witness_and_hash();
...
impl<C: ConstraintSynthesizer<Fq>> Add<R1CS<C>> for R1CS<C> {
    type Output = Self;
...
impl<C: ConstraintSynthesizer<Fq>> AddAssign<R1CS<C>> for R1CS<C> {
