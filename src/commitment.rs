//! Commitment logic used for the creation of committed circuit structures.

use ark_bls12_381::{Fq, G1Projective};
use ark_ec::CurveGroup;
use ark_ff::{PrimeField, UniformRand};
use rand_core::OsRng;
use rayon::prelude::*;

// TODO: this is janky and should be updated
pub fn create_generators(n: usize) -> Vec<G1Projective> {
    let cap = n.next_power_of_two();
    let mut gens: Vec<G1Projective> = Vec::with_capacity(cap);
    for _ in 0..cap {
        gens.push(G1Projective::rand(&mut OsRng {}));
    }
    gens
}

pub fn commit<G: CurveGroup>(generators: &[G], scalars: &[Fq]) -> G {
    scalars
        .par_iter()
        .zip(generators)
        .map(|(scalar, gen)| gen.mul_bigint(scalar.into_bigint()))
        .reduce(|| G::zero(), |a, b| a + b)
}
